...
Status: **Proposed** · measured against `main` at **`a8ca47b`**, 2026-08-08.
...
1. **Twelve surfaces are shipped and running on `main`** — S1, S2, **S3**, S4 (render,
   why-it-matters, submit — three), S5, S6, S7, S13, System Status, and the local MCP surface. ⚠️ Several ship a
...
3. **The second obstacle is one undischarged precondition** — **ADR-0055 D7**, the operator's own
   onboarding run — which blocks S11 entirely. ⚠️ It no longer caps S3: #257 shipped that screen
   **without** reading anything persisted, and 🚫 without claiming anything it had not read. ⚠️ It no longer blocks S5: the BIF
